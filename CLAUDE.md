...
이 프로젝트에서는 AI가 코드를 작성하더라도 **기존 Hexagonal/Layered 구조와 의존성 방향을 절대 우회하지 않는다.**

### 기본 원칙

* 기존 아키텍처와 패키지 구조는 변경 대상이 아니라 제약 조건이다.
* 요청받지 않은 리팩터링은 하지 않는다.
* 요구사항을 해결하는 데 필요한 범위만 수정한다.
* 기존 동작, public API, 패턴은 명확한 필요가 없는 한 유지한다.
* 새로운 구조나 패턴보다 프로젝트 내부의 기존 구현 방식을 우선한다.

### 추상화와 공통화

* 미래 확장성만을 이유로 interface, abstraction, generic framework를 만들지 않는다.
* 실제로 여러 구현이 존재하거나 외부 경계 분리가 필요한 경우에만 추상화한다.
* 작은 중복은 잘못된 공통화보다 낫다.
* `common`은 단순히 코드가 비슷하다는 이유로 사용하지 않는다.
* locality를 우선하되, 이를 위해 레이어 경계를 위반하지 않는다.

### 구현 방식

* 문제의 원인을 직접 수정하고 wrapper, fallback, compatibility layer를 불필요하게 덧붙이지 않는다.
* 과도한 null 처리, blanket `try-catch`, 현실적으로 발생하지 않는 상황을 위한 방어 코드는 피한다.
* 비즈니스 규칙은 Domain/Application에 두고 Controller나 Repository에 우회해서 넣지 않는다.
* 모든 레이어를 반드시 수정하지 않는다. 필요한 레이어만 변경한다.
* 한 작업에서 unrelated 리팩터링이나 이름 변경을 같이 하지 않는다.

### 기존 코드 우선

구현 전에 같은 종류의 기존 코드를 먼저 확인한다.
...
기존 패턴이 있으면 새로운 스타일을 만들지 않는다.

또한 실제 코드를 확인하지 않고 Entity 필드, Repository 메서드, Event, 설정값 등을 추측해서 만들지 않는다.

### 테스트

* 새 기능에는 테스트를 작성한다.
* 버그 수정 시 가능하면 regression test를 추가한다.
* 내부 구현보다 비즈니스 규칙, 상태 변화, 외부 observable behavior를 검증한다.
* 테스트를 통과시키기 위해 비즈니스 로직을 왜곡하거나 기존 테스트를 삭제하지 않는다.

작업 후 변경 범위에 맞게 다음을 실행한다.
...
* 기존 Hexagonal/Layered 구조 재설계
* Vertical Slice 등 다른 아키텍처 임의 적용
* 요청하지 않은 대규모 리팩터링
* 사용처 하나뿐인 범용 abstraction 생성
* 모든 클래스에 interface 생성
* 작은 중복을 제거하기 위한 premature abstraction
* 레이어 경계를 건너뛰는 직접 호출
* 기존 코드 확인 없이 API나 구조 추측
* 미래 확장성을 이유로 현재 복잡도 증가
...
핵심은 **AI가 프로젝트를 다시 설계하지 않고, 현재 구조 안에서 필요한 변경만 정확하게 수행하는 것**이다.
